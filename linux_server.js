...
app.post('/obfuscate', (req, res) => {
    ...
    if (!code || !platform) return res.status(400).json({ error: 'Missing code or platform parameter.' });
    if (platform !== 'Linux') return res.status(400).json({ error: 'This server only handles Linux requests.' });
    ...
    console.log(`[NIX][${sessionId}] Executing: ${command}`);
    
    exec(command, { cwd: __dirname }, (error, stdout, stderr) => {
        const fullOutput = stdout + stderr;
        if (error) {
            console.error(`[NIX][${sessionId}] Script failed:`, error);
            rimraf(sessionDir, () => {});
...
app.get('/download/:sessionId/:binaryName', (req, res) => {
    ...
    res.download(binaryPath, binaryName, (err) => {
        if (err) console.error(`[NIX][${sessionId}] Download Error: ${err.message}`);
        rimraf(sessionDir, () => {});
    });
});

app.listen(PORT, () => console.log(`Chakravyuha Linux backend running on http://localhost:${PORT}`));